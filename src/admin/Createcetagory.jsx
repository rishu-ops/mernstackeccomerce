...
const CreateCategory = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const { data } = await axios.post("http://localhost:8000/api/vl/category/create-category", {
        name,
      });
      if (data?.success) {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const { data } = await axios.put(
        `http://localhost:8000/api/vl/category/update-category/${selected._id}`,
        { name: updatedName }
      );
      if (data.success) {
  ...
  const handleDelete = async (categoryId) => {
    try {
      const { data } = await axios.delete(
        `http://localhost:8000/api/vl/category/delete-category/${categoryId}`
      );
      if (data.success) {
